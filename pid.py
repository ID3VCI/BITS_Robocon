...
class Controller:
    ...
    def mover(self):
        ...
        print("\n")
        Xtarget=float(input("X = "))
        Ytarget=float(input("Y = "))
        ...
        while not rospy.is_shutdown():
            ...
            if(X>0 and Y>0):
                THETA = atan(Y/X)*180/math.pi
            elif(X<0 and Y>0):
                THETA = atan(Y/X)*180/math.pi + 180
            elif(X<0 and Y<0):
                THETA = atan(Y/X)*180/math.pi - 180
            elif(X>0 and Y<0):
                THETA = atan(Y/X)*180/math.pi 

            self.vel_msg.angular.z= (THETA-self.theta)/10 


            if(abs(THETA-self.theta)<0.1):
                self.vel_msg.linear.x=d
            else:
